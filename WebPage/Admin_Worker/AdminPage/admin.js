...
function renderItemsTable(items) {
    ...
    const headers = ['ItemId', 'CategoryId', 'Terméknév', 'Mennyiség', 'Leírás', 'Ár'];
...
async function saveItemRow(row, btn) {
  ...
  body: JSON.stringify({
    itemId: id,
    categoryName: row.querySelector('.cell-categoryid').textContent.trim(),
    itemName: name,
    quantity,
    description,
    price
...
/* Termék módosítás*/
document.getElementById('modify-item-form')
  ?.addEventListener('submit', async e => {
    e.preventDefault();
    const form = e.target;
    if (!validateForm(form)) return;
...
/* Termék törlés*/
document.getElementById('delete-item-form')
    ?.addEventListener('submit', async e => {
        e.preventDefault();
        const form = e.target;
        if (!validateForm(form)) return;
